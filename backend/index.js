...
app.post("/create-account", async (req, res) => {
    ...
    if (!fullName || !email || !password) {
        return res.status(400).json({ error: true, message: "Details are required." });
    }
...
app.post("/add-note", authenticateToken, async (req, res) => {
    ...
    const { user } = req.user; // Fix destructuring
    ...
    try {
        const note = new Note({
            ...
            content,
            tags: tags || [],
            userId: user._id, // Use user._id from the token
        });
...
app.put("/edit-note/:noteId", authenticateToken, async (req, res) => {
    ...
    if(!title || !content || !tags) {
        return res.status(400).json({
            error: true,
            message: "No updates are provided."
        });
    }
